...
inputTextFile  = "__texts_selected/0902Sakhawi.DawLamic.JK003608-ara1.EIS1600"
outputTextFile = inputTextFile
textFileIDs    = inputTextFile +".IDs"

IDsFullList    = "./misc/generate_IDs/IDs_NUMBERS_L5_T100000.txt"
...
def loadIDsFullList(IDsFullList):
    with open(IDsFullList, "r", encoding="utf8") as f1:
        IDs = f1.read().split("\n")
    return(IDs)

def processIDs(inputTextFile, IDsFullList):
    print("="*80)
    print("Processing IDs in: %s" % inputTextFile.split("/")[-1])
    print("="*80)

    outputTextFile  = inputTextFile#+".TEST"
    IDsFullList = loadIDsFullList(IDsFullList)
...
processIDs(inputTextFile, IDsFullList)
